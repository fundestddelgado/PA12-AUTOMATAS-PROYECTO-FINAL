...
def train_binary(df: pd.DataFrame):
    # Entrena varios clasificadores y elige el de mayor AUC en un hold-out 80/20
    ...
    models = {
        'random_forest': RandomForestClassifier(n_estimators=300, max_depth=None, random_state=42, class_weight='balanced'),  # robusto a no linealidades y desbalance
        'gradient_boosting': GradientBoostingClassifier(random_state=42),  # captura interacciones moderadas
        'logistic_regression': LogisticRegression(max_iter=2000, class_weight='balanced', random_state=42)  # baseline lineal
    }
    ...
    # Selección por mayor AUC; ignora modelos sin AUC y en empate usa el primero en orden
    candidates = [(k, v) for k, v in aucs.items() if v is not None]
    best_name = sorted(candidates, key=lambda x: x[1], reverse=True)[0][0]
    best_model = fitted[best_name]
    
    # Guardar métricas (no sobreescribir modelo de producción aquí; el reemplazo depende del llamador)
...
def train_multilabel(df: pd.DataFrame):
    ...
    for label in MULTILABEL_COLS:
        ...
        # Validar que la columna de la etiqueta exista
        ...
        # Trabajar solo con filas no nulas para esta etiqueta
        X = X.loc[non_na_mask]
        y = y.loc[non_na_mask]
...
def main():
    ...
    # Reemplazar el modelo de producción solo si la AUC mejora al menos delta (o si no existe modelo previo)
    new_auc = aucs.get(best_name)
    replace = False
    if current_auc is None:
        # Si no existe AUC previa, reemplazar
        replace = True
    elif new_auc is None:
        replace = False
    else:
        # Reemplazar solo si mejora al menos el delta configurado
