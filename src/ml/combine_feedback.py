...
def load_predictions(pred_path):
    ...
    # Soportar columnas 'machine_failure' o 'Machine failure'
...
def upgrade_pred_log(pred_path):
    """Normaliza y actualiza un predicciones.csv existente: mantiene encabezados, tipos y timestamps consistentes.
    Reescribe el CSV en sitio con columnas y formatos canónicos.
    """
    if not os.path.exists(pred_path):
        return False
    ...
    if 'Machine failure' in df.columns and 'machine_failure' not in df.columns:
        df['machine_failure'] = df['Machine failure']
    # Normalizar columnas de timestamp
    try:
        df['timestamp'] = pd.to_datetime(df['timestamp'], errors='coerce')
        df['timestamp'] = df['timestamp'].dt.tz_convert(None)
    except Exception:
    ...
    try:
        ...
        df['pred'] = df['pred']
    df['machine_failure'] = pd.to_numeric(df['machine_failure'], errors='coerce')
    # Normalizar timestamp de feedback
...
def combine_predictions_with_feedback(predictions_df, feedback_df, time_window_minutes=5):
    ...
    # Support two modes:
    # - Si se entrega feedback_df, combinar predicciones con feedback por timestamp
...
def save_labeled_data(combined_df, output_dir):
    ...
    # Mantener solo filas cuyo _prediction_timestamp no esté ya en el histórico
