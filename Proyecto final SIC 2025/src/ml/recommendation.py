...
def generate_recommendations(instance: Dict, failure_prob: float, shap_contrib: Dict = None) -> List[Dict]:
    ...
    if torque is not None and rot is not None:
        ...
        if power < 3500:
            recs.append({
                'accion': 'Ajustar torque/velocidad para alcanzar potencia mínima',
                'justificacion': f'Potencia estimada {power:.0f}W < 3500W (riesgo PWF).'
            })
        elif power > 9000:
            recs.append({
                'accion': 'Reducir carga o velocidad para evitar sobrepotencia',
                'justificacion': f'Potencia estimada {power:.0f}W > 9000W (riesgo PWF).'
            })
    ...
    if prod_type and torque is not None and wear is not None:
        ...
        if strain > limit:
            recs.append({
                'accion': 'Reducir torque o reemplazar herramienta (sobrestrain)',
                'justificacion': f'Strain {strain:.0f} min*Nm > {limit} límite {prod_type} (OSF).'
            })
    ...
    if failure_prob >= 0.5:
        ...
        recs.append({
            'accion': 'Activar inspección preventiva',
            'justificacion': f'Probabilidad fallo {failure_prob:.2f} >= 0.50' + (f'. Feature crítica: {top}' if top else '')
        })
    
    if not recs:
        recs.append({
            'accion': 'Operar normalmente',
            'justificacion': 'Parámetros dentro de rangos seguros.'
        })
