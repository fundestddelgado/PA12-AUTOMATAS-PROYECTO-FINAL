...
def test_binary_model_not_replaced_if_auc_higher():
    ...
    try:
        ...
        with open(prod_model_path, 'rb') as f:
            new_bytes = f.read()
        assert orig_bytes == new_bytes, "El modelo en producción fue reemplazado a pesar de tener un AUC existente más alto"
    finally:
...
def test_multilabel_skips_labels_with_nan_or_single_class():
    # Construir pequeño dataset de demostración
    n = 100
    df = pd.DataFrame({
        ...
        'twf': [np.nan] * n,  # todos NaN -> debe omitirse
        'hdf': [0] * n,  # clase única -> debe omitirse
        'pwf': np.random.binomial(1, 0.1, n),  # etiqueta válida
