...
def _to_original_schema(df_row: pd.DataFrame, base_file: str):
    """Convierte una fila normalizada al esquema original de ai4i2020.csv.
    Usa el encabezado del CSV base como referencia de columnas y remapea los nombres normalizados.
    """
    try:
...
def bulk_log(df: pd.DataFrame):
    # Requiere que exista la columna failure_prob
...
def main():
    ...
    st.caption('Monitoreo predictivo de centros CNC para anticipar fallos, evitar paros y priorizar acciones en planta.')
...
""")
    ...
    with tab_info:
        ...
        st.markdown('''**Automatas** es un sistema inteligente de mantenimiento predictivo para centros de mecanizado que anticipa fallos en herramientas y máquinas mediante análisis de parámetros operativos, permitiendo intervenciones proactivas que reducen paros no planificados y optimizan costos de operación.''')
